...
from wsmol.methods.grad_cam import GradCAM
...
from .methods import ADL, AcolBase, spg
from .methods.util import normalize_tensor
from .util import initialize_weights, remove_layer, replace_layer
...
class VggGradCam(nn.Module):
    def __init__(self, pretrained, classif_type="multi_class", num_classes=80, **kwargs):
        ...
        self.grad_cam = GradCAM(
            self.model, target_module=self.model.model.avgpool)
...
class VggCam(nn.Module):
    def __init__(self, features, classif_type="multi_class", num_classes=1000, **kwargs):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        if return_cam:
            if self.classif_type == "multi_class":
                ...
                cams = (cam_weights.view(
                    *feature_map.shape[:2], 1, 1) * feature_map).mean(1, keepdim=False)
            else:
                b, c = labels.shape
                feature_map = x.detach().clone()
...
def load_pretrained_model(model, wsol_method, path=None):
...
def vgg16(wsol_method, pretrained=False, pretrained_path=None,
          **kwargs):
    config_key = '28x28' if kwargs['large_feature_map'] else '14x14'
    layers = make_layers(configs_dict[wsol_method][config_key], **kwargs)
    model = {'cam': VggCam,
             'acol': VggAcol,
             'spg': VggSpg,
             'adl': VggCam}[wsol_method](layers, **kwargs)
    if pretrained:
        model = load_pretrained_model(model, wsol_method,
                                      path=pretrained_path)
    return model


def vgg16gnn(wsol_method, classif_type, num_classes, pretrained=False, pretrained_path=None, **kwargs):
    model = {'grad_cam': VggGradCam}[wsol_method](
        pretrained, classif_type, num_classes)
    if pretrained:
        model = load_pretrained_model(model, wsol_method,
                                      path=pretrained_path)
    return model
