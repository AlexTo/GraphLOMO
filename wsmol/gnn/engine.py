...
class MultiLabelEngine(object):
    ...
    def on_start_epoch(self, training, model, criterion, data_loader, optimizer=None, scheduler=None, display=True):
        self.state['meter_loss'].reset()
        self.state['batch_time'].reset()
        self.state['data_time'].reset()
        self.state['ap_meter'].reset()

    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, scheduler=None, display=True):
    ...
    def on_start_batch(self, training, model, criterion, data_loader, optimizer=None, scheduler=None, display=True):
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, scheduler=None, display=True):
        # record loss
    ...
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, scheduler=None, display=True):
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None, scheduler=None):
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset,
                                                   ...
                                                   num_workers=self.state['workers'], pin_memory=True, drop_last=True)
        ...
        if self.state['lr_scheduler']:
            scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=self.state['lr'], steps_per_epoch=len(train_loader), epochs=self.state['max_epochs'], pct_start=0.2)

        for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
            self.state['epoch'] = epoch
            lr, decay = self.adjust_learning_rate(optimizer)
            print('lr:',lr, '|', 'step:' ,self.state['epoch_step'],'|', 'decay: ', decay)
            if scheduler is not None:
                print('lr_scheduler:', scheduler.get_last_lr()[0])

            # if need to reload the checkpoint
            if decay != 1.0 and self.state['lr_best'] and self.state['filename_previous_best'] is not None:
                print("=> loading checkpoint '{}'".format(self.state['filename_previous_best']))
            ...
            # train for one epoch
            self.train(train_loader, model, criterion, optimizer, scheduler, epoch)
    ...
    def run(self, training, data_loader, model, criterion, optimizer=None, scheduler=None, epoch=None):
        if training:
        ...
        self.on_start_epoch(training, model, criterion, data_loader, optimizer, scheduler)
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            self.on_start_batch(training, model, criterion, data_loader, optimizer, scheduler)

            if self.state['use_gpu']:
                self.state['target'] = self.state['target'].cuda()

            self.on_forward(training, model, criterion, data_loader, optimizer, scheduler)
            ...
            # measure accuracy
            self.on_end_batch(training, model, criterion, data_loader, optimizer, scheduler)

        return self.on_end_epoch(training, model, criterion, data_loader, optimizer, scheduler)

    def train(self, data_loader, model, criterion, optimizer=None, scheduler=None, epoch=None):
        return self.run(True, data_loader, model, criterion, optimizer, scheduler, epoch)
    ...
    def save_checkpoint(self, state, is_best, filename='checkpoint.pth'):
        ...
        if is_best:
            ...
            if self.state['save_model_path'] is not None:
                ...
                filename_best = os.path.join(self.state['save_model_path'], 'model_best_{score:.4f}.pth'.format(score=state['best_score']))
                shutil.copyfile(filename, filename_best)
