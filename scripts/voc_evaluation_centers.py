...
def get_args():
    ...
    parser.add_argument('--data_dir', default="./dataset/VOC")
    parser.add_argument('--experiment_name',
                        default="VOC_resnext50_grad_cam_91.5624")
...
def mean_average_precision(predictions, image_sizes, gt_bboxes, image_size, cob_threshold=0):

    def resize_bbox(row):
        new_w, new_h = 448, 448
        orig_w, orig_h = image_sizes[row["image_id"]]
...
def score():
    ...
    image_sizes = {}
    with open("metadata/VOC/val/image_sizes.txt") as f:
        lines = f.readlines()
        for l in lines:
            arr = l.strip().split(",")
            image_sizes[arr[0]] = list(map(int, arr[1:]))

    gt_bboxes = []
    with open("metadata/VOC/val/localization.txt") as f:
        lines = f.readlines()
        for i, l in enumerate(lines):
            ...
            gt_bboxes.append({
                "gt_id": str(i),
                "image_id": arr[0],
                "category_id": int(arr[1]),
                "bbox": [int(arr[2]), int(arr[3]),  int(arr[4]) - int(arr[2]),  int(arr[5]) - int(arr[3])],
            })
    ...
    for threshold in [30, 50, 70]:
        ...
        CP, CR, APs, mAP = mean_average_precision(
            predictions, image_sizes, gt_bboxes, args.image_size)
