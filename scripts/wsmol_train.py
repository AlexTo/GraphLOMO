...
dataset_mappings = {
    "coco": {
        "dataset_class": COCO2014,
        "num_classes": 80
    },
    "voc": {
        "dataset_class": Voc2007Classification,
        "num_classes": 20
    }
}
...
parser.add_argument('--data-dir',
                    help='path to dataset (e.g. dataset/COCO')
parser.add_argument('--metadata-dir',
                    help='path to dataset metadata (e.g. metadata/COCO')
...
parser.add_argument('--epoch_step', default=[], type=int, nargs='+',
...
parser.add_argument('--lr', '--learning-rate', default=0.0045, type=float,
                    metavar='LR', help='learning rate for visual representation learning')
parser.add_argument('--lrg', '--learning-rate-graph', default=0.045, type=float,
                    metavar='LR', help='learning rate for graph representation learning')
parser.add_argument('--lrd', '--learning-rate-decay', default=0.1, type=float,
                    metavar='LRD', help='learning rate decay')
parser.add_argument('--lrb', '--learning-rate-best', dest='lrb', action='store_true',
                    help='use the previous best when lr is changed')
parser.add_argument('--lrs', '--learning-rate-scheduler', dest='lrs', action='store_true',
...
parser.add_argument('--embedding', type=str, metavar='EMB',
...
parser.add_argument('-o', '--optim', dest='optim', default='SGD', type=str, metavar='O',
                    help='Optimizer (default: SGD)')
parser.add_argument('-a', '--arch', dest='arch', default='resnet50', type=str, metavar='A',
                    help='backbone architecture to be used (default: resnet50)')
parser.add_argument('-aug', '--augmentation', dest='aug', action='store_true',
                    help='use rand + cutout')
parser.add_argument('-g', '--graph', dest='graph', action='store_true',
                    help='use graph convolutions')
...
parser.add_argument('-t', '--adj-threshold', default=0.4, type=float, metavar='T',
                    help='Adj threshod (default: 0.4')
parser.add_argument('-adj', '--adj-files', default=['metadata/COCO/topology/coco_adj.pkl'], type=str, nargs='+',
                    help='Adj files (default: [coco_adj.pkl])')
parser.add_argument('-n', '--exp-name', dest='exp_name', default='coco', type=str, metavar='COCO',
                    help='Name of experiment to have different location to save checkpoints')
parser.add_argument('-nt', '--neptune', dest='neptune', action='store_true', help='run with neptune')
parser.add_argument('--neptune_path', default=".neptune", help='path to neptune config file')
...
def main():
    ...
    model = build_net(num_classes=num_classes, arch=args.arch, graph=args.graph, gtn=args.gtn, t=args.adj_threshold,
                      adj_files=args.adj_files, emb_features=args.embedding_length)

    criterion = nn.MultiLabelSoftMarginLoss()

    optimizer = torch.optim.SGD(model.get_config_optim(
        args.lr, args.lrg), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
